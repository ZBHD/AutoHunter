...
**Architecture:** `SystemSettings.fofa_keys` 保存有序凭据和可恢复运行状态；`FofaKeyRouter` 只负责候选选择、单圈重试和状态变更通知；`settings_service` 负责脱敏 CRUD、指纹保护和事务持久化。任务级 `fofa_config.key` 通过单 Key Router 保持显式覆盖，池为空时继续回退旧 `fofa.key`、`engines.fofa.key` 或 `FOFA_KEY`。
...
- Create: `tests/test_fofa_key_config.py` - Key 模型、旧库迁移、缓存解析测试。
...
- Modify: `app/config.py` - 增加 `FofaKeyConfig`。
...
Implement `resolve_fofa_keys(task=None)` so task `fofa_config.key` returns a single `Task override` entry; a stored nonempty pool wins; an empty pool synthesizes read-only legacy behavior at the API layer from `fofa.key`, `engines.fofa.key`, then `FOFA_KEY`.
...
class FofaKeyRouter(Generic[T]):
    ...
    def execute_sync(self, operation: Callable[[str], T]) -> T:
        return self._execute_sync_ring(operation)

    async def execute_async(self, operation: Callable[[str], Awaitable[T]]) -> T:
...
`public_settings_view()` returns `fofa_keys`; `run_settings_health_check()` returns `fofa_results` for pool mode and retains `fofa_result` for legacy mode. Apply probe results atomically only when the Key fingerprint still matches.
...
Build the Router on the orchestrator event loop before dispatching Worker/Killsweep to threads, matching the LLM disable callback pattern. Preserve task override behavior and the current `fofa_base_url`; remove long-lived global raw Key injection after all call sites migrate.
...
Replace the single global FOFA Key input with `<FofaKeysPanel ref="fofaKeyPanel" @change="fofaKeys = $event" @mutated="markHealthStale" />`. Keep `fofa_base_url`, `max_pages`, `page_size`, and `default_intent_mode` in the FOFA settings block. In `runHealthCheck()`, call both `providerPanel.applyHealthCheck(response)` and `fofaKeyPanel.applyHealthCheck(response)`.
