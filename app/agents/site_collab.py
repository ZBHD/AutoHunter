...
_USER_CRED_DIRECTIVE = (
    ...
    "登记后 http_request 会自动携带登录态，不必每次手动拼 Cookie。若是表单/CAS/SSO，先 GET 登录页取隐藏字段（lt/execution 或 csrf token），"
    "再带字段 POST，且 follow_redirects=true；一次请求走完 302 连环跳，结合 redirect_chain/final_url 判成败。\n"
